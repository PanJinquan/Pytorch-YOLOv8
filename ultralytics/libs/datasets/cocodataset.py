...
from pybaseutils.dataloader import parser_coco_ins, parser_coco_kps
...
class COCODataset(YOLODataset):
    ...
    def __init__(self, *args, data=None, task="detect", **kwargs):
        ...
        if self.use_keypoints:
            self.data_parser = parser_coco_kps.CocoKeypoints(anno_file,
                                                             image_dir=None,
                                                             class_name=self.class_dict,
                                                             decode=False,
                                                             check=self.check)
        else:
            self.data_parser = parser_coco_ins.CocoInstances(anno_file,
                                                             image_dir=None,
                                                             class_name=self.class_dict,
                                                             decode=False,
                                                             check=self.check)
    ...
    def get_labels(self):
        ...
        for i in range(len(self.data_parser.image_ids)):
            ...
            boxes, cls, mask, segs = data_info['boxes'], data_info['labels'], data_info['mask'], data_info['segs']
